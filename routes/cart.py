...
async def read_user_cart(user_id: str):
    cart_items = list(get_cart_collection(user_id).find())
    if not cart_items:
...
async def add_to_cart(user_id: str, cart_data: CartModel):
    cart_item = get_cart_collection(user_id).find_one({"product_id": cart_data.product_id})
    if cart_item:
...
async def empty_cart(user_id: str):
    result = get_cart_collection(user_id).delete_many({})
    if result.deleted_count == 0:
...
async def decrease_quantity(user_id: str, product_id: str):
    cart_item = get_cart_collection(user_id).find_one({"product_id": product_id})
    if not cart_item:
    ...
    if cart_item['quantity'] > 1:
        ...
        get_cart_collection(user_id).delete_one({"product_id": product_id})
        return {"message": "Item removed from cart"}